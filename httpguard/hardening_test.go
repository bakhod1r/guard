...
func TestCookieUnsafeRequestsRequireTrustedOrigin(t *testing.T) {
	...
	e.router.Handle("POST /opt", httpguard.Authenticate(e.g, httpguard.Options{})(
		http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			_ = json.NewEncoder(w).Encode(map[string]any{"authed": httpguard.PrincipalFrom(r) != nil})
		})))
	httpguard.Mount(e.router, e.g, httpguard.Options{AuthPath: "/t/auth", AdminPath: "/t/guard", AuthRateLimit: ratelimit.Rule{Limit: -1},
...
func TestSecurityHeaders(t *testing.T) {
	e := newEnv(t, envConfig{})
	httpguard.Mount(e.router, e.g, httpguard.Options{AuthPath: "/h/auth", AdminPath: "/h/guard", HSTS: true, AuthRateLimit: ratelimit.Rule{Limit: -1}})
	...
	t.Run("standalone without options", func(t *testing.T) {
		h := httpguard.SecurityHeaders()(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestBodyLimit(t *testing.T) {
	e := newEnv(t, envConfig{})
	httpguard.Mount(e.router, e.g, httpguard.Options{AuthPath: "/s/auth", AdminPath: "/s/guard", MaxBodyBytes: 64, AuthRateLimit: ratelimit.Rule{Limit: -1}})
...
func TestLoginRevokesPreviousCookieSession(t *testing.T) {
	...
	t.Run("revoke failure is logged, login still succeeds", func(t *testing.T) {
		var logged []error
		httpguard.Mount(e.router, e.g, httpguard.Options{AuthPath: "/l/auth", AdminPath: "/l/guard", AuthRateLimit: ratelimit.Rule{Limit: -1},
...
func TestErrorLoggerReceivesInternalErrorsWithoutLeaking(t *testing.T) {
	e := newEnv(t, envConfig{})
	var logged []error
	httpguard.Mount(e.router, e.g, httpguard.Options{AuthPath: "/e/auth", AdminPath: "/e/guard", AuthRateLimit: ratelimit.Rule{Limit: -1},
		ErrorLogger: func(_ *http.Request, err error) { logged = append(logged, err) }})
	...
	t.Run("ErrorLogging wraps standalone limiter", func(t *testing.T) {
		...
		h := httpguard.ErrorLogging(func(_ *http.Request, err error) { got = err })(
			httpguard.RateLimit(g, httpguard.Options{}, "x", ratelimit.Rule{Limit: 1, Window: 1e9}, httpguard.ByIP)(
				http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusNoContent) })))
		e.router.Handle("GET /logged-rl", h)
